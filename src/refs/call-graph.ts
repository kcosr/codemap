...
const CALL_KINDS: ReferenceKind[] = ["call", "instantiate"];
...
export function buildCallersGraph(
  db: CacheDB,
  rootId: number,
  maxDepth = 3,
): CallGraphNode {
...
export function renderCallGraph(node: CallGraphNode): string {
  const lines: string[] = [];

  const walk = (cur: CallGraphNode, depth: number) => {
...
function buildNode(
  ...
  const rows = db.listReferenceRows(direction, { symbolId }, CALL_KINDS);
  const childMap = new Map<string, CallGraphNode>();
...
function formatNodeLabel(node: CallGraphNode): string {
